...
send_error_email() {
    ...
    if [[ -z "$EMAIL_TO" || -z "$SMTP_SERVER" || -z "$SMTP_USER" || -z "$SMTP_PASS" ]]; then
        echo "⚠️ Email notification skipped: SMTP parameters are not fully configured in run_pipeline_deeplabv3p.sh."
        return 0
    fi
...
try:
    ...
    msg['From'] = "${EMAIL_FROM}"
    msg['To'] = "${EMAIL_TO}"
    msg['Subject'] = "❌ [LULC DeepLabV3+ Pipeline Failure] ${stage_name}: ${failed_step} Failed"

    body = """
Attention,

An error occurred in the LULC Segmentation DeepLabV3+ training pipeline.
...
run_stage() {
    ...
    echo "=========================================================================="
    echo "▶️ STARTING DeepLabV3+ PIPELINE STAGE: $stage_name"
    echo "=========================================================================="
    ...
    echo "✅ DeepLabV3+ Pipeline Stage '$stage_name' finished successfully!"
    echo "=========================================================================="
    echo
}

# ==============================================================================
# DeepLabV3+ PIPELINE EXECUTION FLOW
# ==============================================================================

# Stage 1 — Baseline (IRSAMap only)
run_stage "Stage_1_DeepLabV3P_Baseline" \
          "configs/irsamap_deeplabv3p_baseline.yaml" \
          "best_irsamap_deeplabv3p_baseline.pth"

# Stage 2 — Support training (IRSAMap + LoveDA support)
run_stage "Stage_2_DeepLabV3P_Support" \
          "configs/irsamap_deeplabv3p_support.yaml" \
          "best_irsamap_deeplabv3p_support.pth"

# Stage 3 — Fine-tuning (on IRSAMap only from Stage 2 checkpoint)
run_stage "Stage_3_DeepLabV3P_Finetuning" \
          "configs/irsamap_deeplabv3p_finetune.yaml" \
          "best_irsamap_deeplabv3p_finetune.pth"

echo "🎉 All 3 DeepLabV3+ pipeline stages (Train → Evaluate → Predict) finished successfully!"
