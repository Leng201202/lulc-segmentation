# Adapted from GeoSeg (https://github.com/WangLibo1995/GeoSeg)
import torch
...
class SeparableConvBNReLU(nn.Sequential):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, dilation=1, norm_layer=nn.BatchNorm2d):
        super().__init__(
            nn.Conv2d(
                in_channels, in_channels, kernel_size, stride=stride, dilation=dilation,
                padding=((stride - 1) + dilation * (kernel_size - 1)) // 2,
...
class GlobalLocalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out + local
        out = self.proj(self.pad_out(out))
        return out[:, :, :h, :w]
